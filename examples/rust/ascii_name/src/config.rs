...
pub const DEFAULT_INPUT_STRING: &str = "Umbrella Corp";
...
pub fn agent_workerpool_assignments() -> HashMap<AgentId, Vec<(WorkerId, Vec<ActivityIdAndBuilder>)>> {
    ...
    let ascii_art_activity_to_worker_assignment = create_activity_to_worker_assignment(
        WORKER_ASCII_ART,
        activities
    );
    ...
    let agent_workerpool_assigments = [
        create_agent_workerpool_assignment(
            AGENT_PRIMARY, 
    ...
    let agent_workerpool_assigments = [
        create_agent_workerpool_assignment(
            AGENT_PRIMARY,
            vec![
                ascii_art_activity_to_worker_assignment
            ]
        )
    ]
    .into_iter()
...
pub fn topic_dependencies<'a>() -> Vec<TopicSpecification<'a>> {
    ...
    topics.push(
        TopicSpecification::new::<AsciiArt>(
            GENERATOR_TOPIC,
            vec![
    ...
    for i in 1..NUM_LINES {
        let current_activity_id = ActivityId::new((i+0) as u64);
...
pub fn worker_agent_map() -> HashMap<WorkerId, AgentId> {
    agent_workerpool_assignments()
        ...
        .flat_map(|(agent_id, worker_assignments)| {
            worker_assignments.iter().map(move |(worker_id, _)| (*worker_id, *agent_id))
        })
        .collect()
}
...
pub fn agent_assignments_ids() -> HashMap<AgentId, Vec<(WorkerId, Vec<ActivityId>)>> {
    agent_workerpool_assignments()
        ...
        .map(|(agent_id, worker_assignments)| {
            (
                agent_id,
                worker_assignments.into_iter()
                    .map(|(worker_id, activities)| {
                        (worker_id, activities.into_iter().map(|(activity_id, _)| activity_id).collect())
                    })
                    .collect(),
            )
        })
...
fn create_activity(activity_id: ActivityId, builder: Box<dyn ActivityBuilder>) -> (ActivityId, Box<dyn ActivityBuilder>) {
    (activity_id, builder)
}
    ...
    create_activity(
        ...
        Box::new(move |id| {
            AsciiArtGenerator::build(
                ...
                &format!("{}/generator_to_printer_0", TOPIC_PREFIX)
            )
        })
    )
}
...
fn create_ascii_art_printer_activity(line_index: usize) -> (ActivityId, Box<dyn ActivityBuilder>) {
    ...
    let input_topic = if line_index == 0 {
        ...
        format!("{}/printer_{}_to_{}", TOPIC_PREFIX, line_index-1, line_index)
    };
    
    // Output topic (none for the last printer)
    let output_topic = if line_index < NUM_LINES - 1 {
        Some(format!("{}/printer_{}_to_{}", TOPIC_PREFIX, line_index, line_index+1))
    } else {
        None
    };
    ...
    create_activity(
        activity_id,
        Box::new(move |id| {
            AsciiArtPrinter::build(
                id, 
                line_index,
                &input_topic,
                output_topic.as_deref()
            )
        })
    )
}
...
fn create_activity_to_worker_assignment(worker_id: WorkerId, activities: Vec<(ActivityId, Box<dyn ActivityBuilder>)>) -> WorkerAssignment {
    (worker_id, activities)
}
 
/// Helper function to create an agent assignment
fn create_agent_workerpool_assignment(agent_id: AgentId, worker_assignments: Vec<WorkerAssignment>) -> (AgentId, Vec<WorkerAssignment>) {
    (agent_id, worker_assignments)
}
 
fn activity_depends_on(dependencies: &mut ActivityDependencies, activity: ActivityId, required_activity: Option<ActivityId>) {
    if !dependencies.contains_key(&activity) {
        dependencies.insert(activity, Vec::new());
    }
