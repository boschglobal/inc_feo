...
const DEFAULT_FEO_CYCLE_TIME: Duration = Duration::from_secs(10);
...
impl Params {
    fn from_args() -> Self {
        ...
        // First argument is the cycle time in milli seconds, e.g. 10000 for 10 seconds
...
mod cfg {
    ...
    use ascii_name::config::{activity_dependencies, agent_workerpool_assignments};
    ...
    pub(super) fn make_config(params: Params) -> PrimaryConfig {
        PrimaryConfig {
            ...
            recorder_ids: vec![],
            worker_assignments: agent_workerpool_assignments().remove(&AGENT_ID).unwrap(),
...
mod cfg {
    ...
    pub(super) fn make_config(params: Params) -> PrimaryConfig {
        let agent_ids: HashSet<AgentId> = agent_workerpool_assignments().keys().copied().collect();
...
mod cfg {
    ...
    pub(super) fn make_config(params: Params) -> PrimaryConfig {
        let activity_worker_map: HashMap<ActivityId, WorkerId> = agent_workerpool_assignments()
            .values()
            .flat_map(|vec| {
                vec.iter()
